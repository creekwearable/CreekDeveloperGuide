...
locale: zh-CN
title: "Flutter 数据库查询与操作"
description: "配置数据库用户并查询本地保存的健康记录。"
platform: Flutter
...
> creat_time ：创建时间
> 
> offset_last：上次接收数据的偏移位置 (秒)
> 
> silent_hr：上次接收数据的偏移位置 (秒)
> 
...
> 
> uploadStatus：上传状态 (0: not uploaded, 1: uploaded)
> 
> ~~hr_interval~~:  [{"/threshold"/:12,"/minute"/:60}]  
> 
> datas : 明细数据 ([{"offset": 1, "value": 10}]) - offset in 秒, value is the heart rate
> 
> 

### 压力 Table Structure (STRESS_DATA_HEAD)
> userID      : 用户 ID
> 
> deviceId     :设备 ID
> 
...
> uploadStatus：上传状态 (0: not uploaded, 1: uploaded)
> 
> datas : 明细数据 ([{"offset": 1, "value": 10}]) - offset in 分钟, value is the stress level
...
> uploadStatus：上传状态 (0: not uploaded, 1: uploaded)
> 
> datas : 明细数据 ([{"offset": 1, "value": 10}]) - offset in 分钟, value is the blood oxygen level
> 
> 

### 睡眠 Table Structure (SLEEP_DATA_HEAD)"
> - userID: 用户 ID
> - deviceId: 设备 ID
...
> - total_sleep_time_mins: Total sleep duration (in 分钟)
> - wake_mins: Total awake duration (in 分钟)
> - light_sleep_mins: Total light sleep duration (in 分钟)
> - deep_sleep_mins: Total deep sleep duration (in 分钟)
> - rem_mins: Total REM sleep duration (in 分钟)
...
> - sleep_score: 睡眠 score
> - datas: 明细数据 ([{"stage": 1, "duration": 70}]) - sleep stage (1-4) and duration in 分钟
...
> 
> \- total_exercise_min: Total exercise duration (in 分钟)
> 
...
> 
> \- total_stand_hour: Total 小时 standing per day
> 
...
> 
> \- datas: 明细数据 ([{"step_count": 1, "exercise_min": 10, "activity_calories": 1, "rest_calories": 10, "distance": 1, "~~stand_time~~": 10, "wear_time": 1}])
> 
...
> \- uploadStatus: 上传状态 (0: not uploaded, 1: uploaded)
> 
> \- datas: 明细数据 ([{"offset": 1, "value": 10}]) - offset in 分钟, value is the heart rate
> 
> 
...
> 
> \- warmUpTime: Accumulated warm-up time (秒)
> 
> \- fatBurningTime: Accumulated fat-burning time (秒)
> 
> \- aerobicExerciseTime: Accumulated aerobic exercise time (秒)
> 
> \- anaerobicExerciseTime: Accumulated anaerobic exercise time (秒)
> 
> \- extremeExerciseTime: Accumulated extreme exercise time (秒)
> 
...
> 
> \- avgKmPace: Average kilometer pace (in 秒, e.g., 361 for 6 分钟 and 1 second)
> 
...
> 
> \- recoveryTime: Recovery time (in 小时)
> 
...
> 
> swimAvgSwolf: Swolf = single lap strokes + lap duration (秒)
> 
...
> 
> swimDurations: Swimming duration; unit: 秒 (s)
> 
> miDistanceSupport: Whether mile distance is supported
> 
> avgMiPace: Average mile pace; unit: 秒 (s)
> 
> fastMiPace: Fastest mile pace; unit: 秒 (s)
> 
...
> 
> totalInterruptTimeSec: Total interruption duration; unit: 秒 (s)
> 
> totalSkipTimeSec: Total jump rope duration; unit: 秒 (s)
> 
...
> 附加字段:
> 
> \- hrValueItem: Real-time heart rate monitoring: Saves a set every 5 秒, with a maximum storage time of 20 小时.
> 
> \- kmSpeedItem: Real-time kilometer speed details (saved every 1 kilometer)
> 
> \- paceCount: Real-time pace details (saved every second)
> 
> \- stepFrequencyItem: Real-time step frequency details (saved every 5 秒)
> 
> \- stepStrideItem: Real-time step stride details (saved every 5 秒)
> 
> -elevationItem: Real-time absolute altitude details Unit: meter Store a value every 2 秒 Data: int16_t
> 
> \- trailData: Sport trajectory - latitude (saved every 2 秒)
> 
> \- speedPaceItem: Real-time speed and pace details with latitude information (saved every 5 秒)
> 
> workoutCourseItem: Workout course details
> 
> 
> 
> powerItem: Real-time power details, stored every 5 秒, data: uint16_t
> 
> runPostureItem：Running Form Details: Saved every 5 秒.
> 
> 
> 
...
> create_time ：
> 
> offset_last：Record the offset position of the last received data.（分钟）
> 
...
> uploadStatus：上传状态 (0: not uploaded, 1: uploaded)
> 
> datas : 明细数据 ([{"offset": 1, "value": 10}]) - offset in 分钟, value is Respiratory
> 
> 
...
> 
> offset_last：last received offset position (分钟)
> 
> min：Minimum blood oxygen level
> 
> max：Maximum blood oxygen level
...
> uploadStatus：上传状态 (0: not uploaded, 1: uploaded)
> 
> datas : 明细数据 ([{"offset": 1, "value": 10}]) - offset in second, value is the blood oxygen level
> 
> 
...
> };
> 
> sleepTempSupport:  睡眠 温度 Support   1:Support. 0 noSupport
> 
> sleepBodyTempAverage:  睡眠 body temperature data, magnified 100 times.
> 
> uploadStatus：上传状态 (0: not uploaded, 1: uploaded)
> 
> datas : 明细数据
