...
每次设置用户信息前，应先读取当前信息，只修改需要变更的字段，再提交完整模型。

## 获取并设置用户信息

```swift
///每次设置用户信息之前需要先获取原来的信息，再改动需要改动的值进行设置
...
CreekInterFace.instance.getUserInfo { model in
    ...
    // 血压单位 0x00：无效, 0x01,毫米汞柱（mmhg）, 0x02,千帕（kpa）
...
| bit 位 | 说明 |
| --- | --- |
| 0 | 是否支持风速单位 `wind_speed_unit` |
| 1 | 是否支持能见度单位 `visibility_unit` |
| 2 | 是否不支持海拔高度单位 `altitude_unit` |
| 3 | 是否支持用户头像 `user_icon_width`、`user_icon_height` |
| 4 | 是否不支持血压单位 `blood_pressure_unit` |
